...
@login_required
def info(request):
    # _n_male = Customers.objects.filter(sex=1).count()
    # _n_female = Customers.objects.filter(sex=0).count()
    ...
    # return render(request, 'demo_app/info.html', {'n_male': _n_male, 'n_female': _n_female})
